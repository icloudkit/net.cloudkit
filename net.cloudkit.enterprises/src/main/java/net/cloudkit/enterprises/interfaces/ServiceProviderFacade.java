...
@WebService(name = "ServiceProviderFacade", targetNamespace = "http://www.cloudkit.net/jax-ws/", serviceName = "ServiceProviderFacade")
...
public interface ServiceProviderFacade {
    ...
    byte[] service(@WebParam(name = "serviceName") String serviceName, @WebParam(name = "contentType") String contentType, @WebParam(name = "requestContext") byte[] requestContext, @WebParam(name = "requestData") byte[] requestData, @WebParam(name = "responseData", mode = WebParam.Mode.OUT) Holder<byte[]> responseData);
}
